...
#Defining function to compute additional payment
def computepay(hrs,rph):
   computed=(hrs*rph)+(hrsleft*(rph*permmulti))
   return computed
...
#Rate per hour
rph = input ('Please enter the rate per hour:')
rph = float(rph)

#Defined permanent mulriplier for rph
permmulti=float(1.5)

#Here we sum stuff up to achieve additional payment needed
